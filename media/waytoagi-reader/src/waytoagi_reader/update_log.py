...
def _render_day(blocks: dict, day_head_id: str) -> dict:
    ...
    for cid in referenced:
        ...
        if item["title"] or item["summary"] or item["type"] in {"image", "divider"}:
            items.append(item)
    if missing or (heading_text and not items):
        ...
        if missing and len(missing) == len(referenced):
            print(
                ...
                "run with --emit-raw-blocks (also --archive for archive reads) "
                f"to inspect missing ids: {', '.join(missing)}.",
                file=sys.stderr,
            )
        elif missing:
            print(
                f"[warn] day heading {heading_text!r} ({day_head_id}) has {len(missing)} "
                f"of {len(referenced)} child block(s) absent from the SSR payload; "
                f"present blocks rendered {len(items)} items. Missing ids: {', '.join(missing)}.",
                file=sys.stderr,
            )
        elif not referenced:
            print(
            ...
            print(
                f"[warn] day heading {heading_text!r} ({day_head_id}) has 0 renderable "
                "children although all referenced blocks are present.",
                file=sys.stderr,
            )
    return {
        ...
        "items": items,
        # Keep the signal on the day so CLI date filtering can exclude it.
        **({"missing_child_ids": missing} if missing else {}),
    }
