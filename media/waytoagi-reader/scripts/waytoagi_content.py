...
def fetch_blocks(url: str, *, use_cache=True):
    ...
    res = cached_fetch(url, fetcher=fetch_html, mode="read+write" if use_cache else "no", ttl=7 * 24 * 3600)
...
def render_page_body(blocks: dict) -> str:
    ...
    def walk(cid, depth=0):
        ...
        b = blocks[cid]
        d = b.get("data") or {}
        # Text-bearing code, ordered lists, table cells, etc. use the same
        # attributed-text contract; containers are traversed even without text.
...
def _batch_chars() -> int:
    budget = int(os.environ.get("WAYTOAGI_BATCH_CHARS", "32000"))
    if budget <= 0:
        raise ValueError("WAYTOAGI_BATCH_CHARS must be positive")
    return budget


def _post(host, model, lines, timeout):
    return post_translation(host, model, _SYSTEM_PROMPT, lines, timeout, 16000)
...
def _translate_text(host, model, text):
    ...
    # Prefer paragraph/word boundaries, but also bound a single oversized block.
    # Keep every source character, including separators, in exactly one chunk.
    chunks = []
    while text:
        end = min(len(text), batch_chars)
        if end < len(text):
            boundary = max(text.rfind("\n", 0, end), text.rfind(" ", 0, end))
            if boundary >= 0:
                end = boundary + 1
            # Never introduce a broken markdown link at a chunk boundary.
            for link in re.finditer(r"\[[^\n]*?\]\([^\s]*\)", text):
                if link.start() < end < link.end():
                    if link.end() - link.start() > batch_chars:
                        raise ValueError("Markdown link exceeds --batch-chars; increase the budget")
                    end = link.start() or link.end()
                    break
    ...
    for c in chunks:
        ...
        for attempt in range(3):
            try:
                ...
                m = re.match(r"\s*\[0\]\s*(.+)$", raw, re.DOTALL)
                translated = m.group(1).strip() if m else raw.strip()
                if not translated or translated == "[0]":
                    raise ValueError("Empty article translation")
        ...
        if not ok:
            raise RuntimeError("Article translation chunk failed after retries")
...
def _content_cache_path(url: str, lang: str) -> Path:
    ...
    return _content_cache_dir() / "v2" / lang / f"{key}.txt"


def _translation_key(url, source, host, model):
    return json.dumps([url, source, host.rstrip('/'), model, _SYSTEM_PROMPT, _batch_chars()], ensure_ascii=False)
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--batch-chars", type=int, default=None,
                    help="Max source chars per translation batch (default env WAYTOAGI_BATCH_CHARS or 32000).")
    ap.add_argument("--limit", type=int, default=int(os.environ.get("WAYTOAGI_MAX_ARTICLES", "0")),
                    help="Max articles to fetch+translate (0 = all). Useful for dry runs.")
    args = ap.parse_args(argv)

    if args.limit < 0 or (args.batch_chars is not None and args.batch_chars <= 0):
        ap.error("--limit must be nonnegative and --batch-chars positive")
    if args.batch_chars is not None:
