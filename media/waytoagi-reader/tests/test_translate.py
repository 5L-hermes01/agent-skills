...
def test_latest_day_only_filters(monkeypatch):
    monkeypatch.setattr(T, "_translate_batch", lambda host, model, texts, use_cache: {i: "Translated" for i, _ in texts})
    doc = {"items": [
        _item("8 月 30 日", "今日标题"),
        _item("8 月 30 日", "另一个"),
        _item("8 月 29 日", "昨日标题"),
...
def test_translate_doc_adds_en_siblings_with_fake_backend(monkeypatch):
    ...
    # This fake translates every input; the real backend's Chinese filter is
    # covered separately, while this test exercises sibling mapping only.
